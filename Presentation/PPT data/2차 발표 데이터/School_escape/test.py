...
class TileMap:
    ...
    SCROLL_KMPH = 10.0  # KM / HOUR , 캐릭터가 이동하는 속도랑 맞춰줌
    ...
    def draw(self):
        ...
        for obj_data in self.obj_data_list:
            draw_rectangle(obj_data[1],obj_data[2],obj_data[3],obj_data[4])
        ...
        for i in self.obj_data_list:
           draw_rectangle(i[1], i[2], i[3], i[4])

    def update(self, frametime):
        self.x = (self.x + frametime * self.speedx) % canvasWidth
        self.y = (self.y + frametime * self.speedy) % canvasHeight

    def handle_events(self, event):
        if event.type == SDL_KEYDOWN:
            if event.key == SDLK_LEFT:
                back.speedx -= back.SCROLL_SPEED_PPS
            elif event.key == SDLK_RIGHT:
                back.speedx += back.SCROLL_SPEED_PPS

            elif event.key == SDLK_UP:
                back.speedy += back.SCROLL_SPEED_PPS
...
open_canvas(800,600)
back = TileMap("Maps\\test_01.json",canvasWidth,canvasHeight)
